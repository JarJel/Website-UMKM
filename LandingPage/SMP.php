<?php
$nama_sekolah = "SMP Raudhatul Jannah";
?>
...
<head>
    ...
    <style>
        ...
        .primary-color {
            background-color: #3D8D7A; /* Diperbarui agar sesuai dengan penggunaan di elemen */
        }

        /* Mengubah definisi text-primary-color agar sesuai dengan warna yang digunakan di elemen */
        .text-primary-color {
            color: #3D8D7A; /* Diperbarui agar sesuai dengan penggunaan di elemen */
        }

        .border-primary-color {
            border-color: #3D8D7A; /* Diperbarui agar sesuai dengan penggunaan di elemen */
        }
...
<body class="bg-gray-200">
    ...
    <div class="pt-[]">
        <section id="beranda" class="text-white py-20 px-6 text-center rounded-b-xl shadow-lg min-h-screen flex items-center justify-center bg-cover bg-center" style="background-image: url('image/background-smp.png');">
            <div class="max-w-4xl mx-auto p-8 rounded-xl">
                ...
                <h1 class="text-4xl md:text-6xl font-extrabold leading-tight mb-6 drop-shadow-md text-[#3D8D7A] fade-in-scale delay-1">Dashboard</h1>
                <h1 class="text-4xl md:text-6xl font-extrabold leading-tight mb-6 drop-shadow-md text-[#3D8D7A] fade-in-scale delay-2"><?php echo $nama_sekolah; ?></h1>
                <!-- Tambahkan id 'mulaiSekarangBtn' pada tombol -->
                <a href="#dashboard" id="mulaiSekarangBtn" class="inline-block bg-[#B3D8A8] text-white font-bold py-3 px-8 rounded-full hover:bg-[#3D8D7A] shadow-lg transition duration-300 bounce-in">Mulai Sekarang</a>
            </div>
        </section>

        <section id="dashboard" class="py-12 px-6 md:px-12 bg-[#3D8D7A] rounded-xl shadow-lg mt-8 mx-4 md:mx-auto max-w-6xl relative">
            ...
            <div class="flex flex-col md:flex-row gap-4 justify-center items-center relative">
                ...
                <div class="bg-white rounded-lg shadow-md p-4 flex flex-col items-center relative">
                    <img src="image/card-smp1.png" alt="Dashboard TK A Grade" class="w-40 h-40 object-cover rounded">
                    <div class="relative mt-2">
                        <!-- Tombol ini akan membuka modal untuk TK A -->
                        <button class="bg-[#3D8D7A] px-3 py-1 text-white rounded text-sm border border-gray-300 open-kelas-modal-btn" data-kelas="7">Pilih Kelas 7</button>
                    </div>
                </div>

                <!-- Card 2: TK B -->
                <div class="bg-white rounded-lg shadow-md p-4 flex flex-col items-center relative">
                    <img src="image/card-smp2.png" alt="Dashboard TK B Grade" class="w-40 h-40 object-cover rounded">
                    <div class="relative mt-2">
                        <!-- Tombol ini akan membuka modal untuk TK B -->
                        <button class="bg-[#3D8D7A] px-3 py-1 text-white rounded text-sm border border-gray-300 open-kelas-modal-btn" data-kelas="8">Pilih Kelas 8</button>
                    </div>
                </div>

                <div class="bg-white rounded-lg shadow-md p-4 flex flex-col items-center relative">
                    <img src="image/card-smp3.png" alt="Dashboard TK B Grade" class="w-40 h-40 object-cover rounded">
                    <div class="relative mt-2">
                        <!-- Tombol ini akan membuka modal untuk TK B -->
                        <button class="bg-[#3D8D7A] px-3 py-1 text-white rounded text-sm border border-gray-300 open-kelas-modal-btn" data-kelas="9">Pilih Kelas 9</button>
                    </div>
                </div>
        ...
        <section id="instagram" class="py-12 px-6 md:px-12 bg-white rounded-xl shadow-lg mt-8 mx-4 md:mx-auto max-w-6xl">
            <h2 class="text-3xl md:text-4xl font-bold text-center text-gray-800 mb-10 drop-shadow-lg">Postingan <span class="text-[#3D8D7A] drop-shadow-lg">Instagram</span></h2>
        ...
        <section id="pendaftaran" class="bg-[#3D8D7A] text-white py-16 px-6 md:px-12 text-center rounded-xl shadow-lg mx-4 md:mx-auto max-w-6xl mt-8">
            ...
            <a href="#kontak" class="inline-block bg-black text-white font-bold py-3 px-8 rounded-full shadow-lg hover:bg-[#3D8D7A] transition duration-500">Hubungi Kami</a>
        </section>

        <!-- Bagian Kontak -->
        <section id="kontak" class="py-16 px-6 md:px-12 bg-white rounded-xl shadow-lg mx-4 md:mx-auto max-w-6xl mt-8 mb-8">
            <h2 class="text-3xl md:text-4xl font-bold text-center text-gray-800 mb-10">Hubungi <span class="text-[#3D8D7A]">Kami</span></h2>
            <div class="flex flex-col md:flex-row justify-center items-start md:space-x-12">
                <div class="md:w-1/2 mb-8 md:mb-0">
                    <p class="text-gray-700 mb-4"><strong class="text-[#3D8D7A] drop-shadow-lg">Alamat:</strong> Jl. Ade Irma Suryani, Labuh Baru, Kec. Payakumbuh Utara, Kota Payakumbuh, Sumatera Barat 26218</p>
                    <p class="text-gray-700 mb-4"><strong class="text-[#3D8D7A] drop-shadow-lg">Telepon:</strong> (0752) 7970355</p>
                    <div class="mt-6">
                        <h3 class="text-xl font-semibold text-gray-800 mb-4">Jam Operasional Kantor:</h3>
                        <ul class="list-disc list-inside text-gray-700">
                            <li>Senin - Sabtu : 07:30 - 16:00 WIB</li>
                            <li>Minggu: Tutup</li>
                        </ul>
                    </div>
                </div>
                <div class="md:w-1/2 w-full">
                    ...
                    <form class="space-y-4" action="submit_message.php" method="POST">
                        ...
                        <input type="hidden" name="unit_asal" value="SMP">
                        <input type="hidden" name="redirect_url" value="smp.php"> <div>
        ...
        <!-- Footer -->
        <footer class="bg-[#3D8D7A] text-white py-8 px-6 md:px-12 text-center rounded-t-xl mt-8">
    ...
    <script>
        ...
        const kelasOptions = {
            '7': [
                { name: 'Kelas AL MULK', url: '' },
                { name: 'Kelas Al Mu’Minun', url: '' },
                { name: 'Kelas AL BALAD', url: '' },
                { name: 'Kelas AN NUR', url: '' },
                { name: 'Kelas AN NAJM (DIGITAL)', url: '' },
                { name: 'Kelas AS SSYAMS (STEM BILINGUAL)', url: '' },
                { name: 'Kelas ASY SYURA (BILINGUAL)', url: '' },
                { name: 'Kelas AL FATH (TAHFIZH)', url: '' },
            ],
            '8': [
                { name: 'Kelas AL BURUJ', url: '' },
                { name: 'Kelas AL ANBIYA', url: '' },
                { name: 'Kelas AZ ZUKHRUF', url: '' },
                { name: 'Kelas AL MA ARIJ', url:''},
                { name: 'Kelas AL BAYYINAH', url:''},
                { name: 'Kelas AL QALAM (DIGITAL)', url:''},
                { name: 'Kelas AL MURSALAT (STEM BILINGUAL)', url:''},
                { name: 'Kelas AS SAJJADAH (TAHFIZH)', url:''},
            ],
            '9': [
                { name: 'Kelas AL A’LA', url: '' },
                { name: 'Kelas AL FAJR', url: '' },
                { name: 'Kelas AL INSYIRAH', url: '' },
                { name: 'Kelas AL QADR', url: '' },
                { name: 'Kelas AL FURQAN', url: '' },
                { name: 'Kelas AL KAUTSAR (DIGITAL)', url: '' },
                { name: 'Kelas AN NASR (STEM BILINGUAL)', url: '' },
                { name: 'Kelas AL A’RAF TAHFIZH', url: '' },
            ]
        };

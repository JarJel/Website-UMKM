    ...
    <main class="flex flex-1 items-center justify-center p-4 sm:p-6 md:p-8">
        <div class="w-full max-w-4xl mx-auto bg-batara-dark-blue rounded-2xl shadow-2xl p-6 sm:p-8">
            <div class="text-center mb-8">
                ...
                <p class="text-batara-lime text-lg font-medium">
                    Bergabunglah dengan komunitas seller terpercaya
                </p>
                        ...
                        <div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input id="nama_pengguna" class="border p-2 bg-gray-100 text-gray-500 shadow-md border-gray-300 rounded-lg w-full cursor-not-allowed" type="text" name="nama_pengguna" value="{{ $user->nama_pengguna }}" readonly disabled />
                                </div>
                                <div>
                                    <label for="email" class="block mb-1 text-gray-200 text-base">Email</label>
                                    <input id="email" class="border p-2 bg-gray-100 text-gray-500 shadow-md border-gray-300 rounded-lg w-full cursor-not-allowed" type="email" name="email" value="{{ $user->email }}" readonly disabled />
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input id="nama_toko" class="border p-2 bg-white text-gray-900 shadow-md placeholder:text-sm focus:scale-105 ease-in-out duration-300 border-gray-300 rounded-lg w-full" type="text" name="nama_toko" placeholder="Masukkan nama toko yang menarik..." required />
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <input id="nomor_telepon" class="border p-2 pl-14 bg-white text-gray-900 shadow-md placeholder:text-sm focus:scale-105 ease-in-out duration-300 border-gray-300 rounded-lg w-full" type="tel" name="nomor_telepon" placeholder="81234567890" required />
                                        <div class="absolute left-4 top-1/2 transform -translate-y-1/2 text-batara-teal font-medium">
                                            +62
                                        </div>
                                    </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input id="ktp" class="border p-2 bg-white text-gray-900 shadow-md border-gray-300 rounded-lg w-full file:mr-4 file:py-1 file:px-3 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-batara-lime file:text-batara-dark-blue hover:file:bg-opacity-80 file:cursor-pointer" type="file" name="ktp" accept="image/*" required />
                                ...
                                <div>
                                    ...
                                    <input id="sku" class="border p-2 bg-white text-gray-900 shadow-md border-gray-300 rounded-lg w-full file:mr-4 file:py-1 file:px-3 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-batara-lime file:text-batara-dark-blue hover:file:bg-opacity-80 file:cursor-pointer" type="file" name="sku" accept="image/*" required />
                                    <p class="text-xs text-gray-400 mt-1">Format: JPG, PNG (maks 2MB)</p>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div id="error-id_provinsi" class="mt-2 text-red-400 text-sm hidden">Provinsi wajib dipilih.</div>
                                </div>
                                <div>
                                    ...
                                    <div id="error-id_kabupaten" class="mt-2 text-red-400 text-sm hidden">Kabupaten wajib dipilih.</div>
                                </div>
                                <div>
                                    ...
                                    <div id="error-id_kecamatan" class="mt-2 text-red-400 text-sm hidden">Kecamatan wajib dipilih.</div>
                                </div>
                                <div>
                                    ...
                                    <div id="error-id_desa" class="mt-2 text-red-400 text-sm hidden">Desa wajib dipilih.</div>
                                </div>
                            </div>
    ...
    provinsiSelect.addEventListener('change', async function () {
        ...
        try {
            ...
            if (Array.isArray(data) && data.length > 0) {
                ...
                data.forEach(item => {
                    kabupatenSelect.insertAdjacentHTML('beforeend', `<option value="${item.id}">${item.name}</option>`);
                });
                kabupatenSelect.disabled = false;
            } else {
                resetSelect(kabupatenSelect, 'Tidak ada kabupaten');
            }
        } catch (err) {
            console.error(err);
    ...
    kabupatenSelect.addEventListener('change', async function () {
        ...
        try {
            ...
            if (Array.isArray(data) && data.length > 0) {
                ...
                data.forEach(item => {
                    kecamatanSelect.insertAdjacentHTML('beforeend', `<option value="${item.id}">${item.name}</option>`);
                });
                kecamatanSelect.disabled = false;
            } else {
                resetSelect(kecamatanSelect, 'Tidak ada kecamatan');
            }
        } catch (err) {
            console.error(err);
    ...
    kecamatanSelect.addEventListener('change', async function () {
        ...
        try {
            ...
            if (Array.isArray(data) && data.length > 0) {
                ...
                data.forEach(item => {
                    desaSelect.insertAdjacentHTML('beforeend', `<option value="${item.id}">${item.name}</option>`);
                });
                desaSelect.disabled = false;
            } else {
                resetSelect(desaSelect, 'Tidak ada desa');
            }
        } catch (err) {
            console.error(err);
    ...
    desaSelect.addEventListener('change', async function () {
        ...
        try {
            ...
            if (Array.isArray(data) && data.length > 0) {
                ...
                data.forEach(item => {
                    bumdesSelect.insertAdjacentHTML('beforeend', `<option value="${item.id_bumdes}">${item.nama_bumdes}</option>`);
                });
